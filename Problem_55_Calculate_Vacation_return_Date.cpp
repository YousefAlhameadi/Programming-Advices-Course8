...
stDate Calc_Total_Vacation_Days(stDate Date1 , short VactionDays)
{

    short WeekEndCounter = 0;

    while (IsWeekend(WeekDays(IndexOfDay(Date1))))
    {
        Date1 = AddDay(Date1);
    }
...
int main()
{
    ...
    VacationStart = GetDate();


    cout << endl << "enter vacation days : ";
    cin >> NumberOfVacationDays;

    stDate VacationEnd = Calc_Total_Vacation_Days(VacationStart,NumberOfVacationDays);

    

    cout << "return Date : " << WeekDays(IndexOfDay(VacationEnd)) << " , " << VacationEnd.Day << " / " << VacationEnd.Month << " / " << VacationEnd.Year;
